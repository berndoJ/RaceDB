...
$sql_query = "SELECT runners.runneruid AS rid, runners.firstname AS rf, runners.surname AS rs, relays.name AS rname FROM runners INNER JOIN relays ON (relays.id = runners.relayid) WHERE runners.relayid IN (SELECT id FROM relays WHERE runid = ?);";
